...
const handleUpvoteClick = (requestId, currentUpvotes) => {
  const currentCount = upvotesCount[requestId] || currentUpvotes;
  if (currentCount === currentUpvotes) {
    // Increment upvote by 1 and change colors
    setUpvotesCount({ ...upvotesCount, [requestId]: currentCount + 1 });
  } else {
    // Decrement upvote by 1 and reset colors
    const updatedUpvotes = { ...upvotesCount };
    delete updatedUpvotes[requestId];
    setUpvotesCount(updatedUpvotes);
  }
};
    ...
    RoadmapData.data.productRequests
      ...
      .map((request) => (
        <div key={request.id} className={`Card--${status}`}>
          ...
          <span className='categories'>{request.category}</span>
          <br />
          <span className={`Numb--upvotes ${upvotesCount[request.id] !== undefined ? 'blue-bg' : ''}`} onClick={() => handleUpvoteClick(request.id, request.upvotes)}>
            <img src={ArrowUp} alt='^' className={`upvotes--icon ${upvotesCount[request.id] !== undefined ? 'white-icon' : ''}`} />
            {upvotesCount[request.id] !== undefined ? upvotesCount[request.id] : request.upvotes}
          </span>
          <div className='Roadmapcard--comments'>
            <img src={CommentsImg} alt='comments' />
